...
public class CollectionManager {
    ...
    public void loadCollectionFromDatabase() {
        ...
        try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
            while (rs.next()) {
                ...
                // Проверяем, есть ли маршрут в коллекции, если нет, то добавляем
                if (routes.stream().noneMatch(r -> r.getId() == id)) {
                    routes.add(route);
                }
            }
    ...
    public void addRoute(Route newRoute) {
        try {
            ...
            routes.add(newRoute);  // Добавляем маршрут напрямую в коллекцию
